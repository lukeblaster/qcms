...
export async function getMagazineLuizaProductInfo(url: any) {
  try {
    ...
    const rawPrice =
      dom.window.document
        .querySelector('p[data-testid="price-value"]')
        ?.textContent?.replace('.', '')
        .replace(',', '.')
        .replace('R$', '')
        .replace('&nbsp', '')
    ...
    // Título do Produto
    const productTitle =
      dom.window.document.querySelector('.jjGTqv')?.textContent ||
      'Produto não disponível'

    // Imagem do Produto
    const imageURL =
      dom.window.document
        .querySelector('img[data-testid="image-selected-thumbnail"]')
        ?.getAttribute('src') ||
      'https://preview.redd.it/x37wd2mzh6n31.png?width=1080&crop=smart&auto=webp&s=d6791e70211c04a331cd14c9ce88ad6a1193f23d'
